...
AGameSlot* AGameGrid::GetSlot(FSGridPosition& Position)
{
    ...
    if (GridActors.IsValidIndex(GridIndex)) return Cast<AGameSlot>(GridActors[GridIndex]->GetChildActor());
    return nullptr;
}

AGameSlot* AGameGrid::FindSlot(FSGridPosition Position)
{
    if (GameGrid) return GameGrid->GetSlot(Position);
    return nullptr;
}
...
void AGameGrid::OnConstruction(const FTransform& Transform)
{
    ...
    for (int i = 0; i < NumRows; i++)
    {
        ...
        {
            FName GridName(FString::Printf(TEXT("Slot%d%d"), j, i));
            auto Grid = NewObject<UChildActorComponent>(this, UChildActorComponent::StaticClass(),
                GridName);
